...
const toCamel = (r) => ({
  ...
  feedUsage: Number(r.feed_usage || 0),
  chickOrderDate: r.chick_order_date,
  chickDeliveryDate: r.chick_delivery_date,
  cullDate: r.cull_date,
  notes: r.notes,
...
router.patch('/:id', async (req, res) => {
  ...
  if (fields.length === 0) {
    const { rows } = await query('select * from public.chickens where id=$1', [id]);
    return res.json(rows.length ? toCamel(rows[0]) : null);
  }

  const sql = `update public.chickens set ${fields.join(', ')}, updated_at=now() where id=$${values.length + 1} returning *`;
  values.push(id);
