...
const notOk = (status = 500, statusText = 'Internal Server Error', body = 'boom', extra = {}) => ({
  ok: false,
  status,
  statusText,
...
describe('apiClient', () => {
  ...
  it('listChickens fetches JSON and returns data', async () => {
    ...
    global.fetch.mockResolvedValueOnce(okJson(rows));
  ...
  it('addChicken posts JSON body and returns created row', async () => {
    ...
    global.fetch.mockResolvedValueOnce(okJson(created));
  ...
  it('throws a helpful error when response is not ok', async () => {
    global.fetch.mockResolvedValueOnce(notOk(404, 'Not Found', 'nope'));
  ...
  it('updateChicken uses PATCH with JSON body and returns row', async () => {
    global.fetch.mockResolvedValueOnce(okJson({ id: 'x1', currentCount: 9 }));
  ...
  it('listFeedLogs GETs logs for a chicken', async () => {
    ...
    global.fetch.mockResolvedValueOnce(okJson(logs));
  ...
  it('updateFeedLog PATCHes JSON and returns updated log', async () => {
    ...
    global.fetch.mockResolvedValueOnce(okJson(updated));
